"""
STL Mesh → 3D Voxel Volume + pmcx Fluence Overlay — 650 nm
-----------------------------------------------------------
Pipeline:
  1. Load one STL per tissue, voxelize via ray casting → integer label volume
  2. Add synovial fluid + soft-tissue wrapping (muscle/adipose/skin/epidermis)
     BEFORE source placement so sources land on the epidermis surface
  3. Build & run pmcx simulation — repeated for three melanin conditions
     (fair / olive / dark Fitzpatrick skin types)
  4. Render with Plotly for each skin condition:
       - One Isosurface per tissue (semi-transparent, colored by tissue)
       - Fluence Volume (Isosurface colormap, log scale) overlaid on top
       - Tissue on/off toggles + fluence source selector in the sidebar
...
# True values from Jacques 2013 / Meglinski-Matcher model.
#
...
#   fair:  1 - exp(-0.020 × 0.2) ≈ 0.4%
#   olive: 1 - exp(-0.070 × 0.2) ≈ 1.4%
#   dark:  1 - exp(-0.200 × 0.2) ≈ 3.9%
_EPI_THICKNESS_MM = 0.2
_EPI_SCALE        = _EPI_THICKNESS_MM / 1.0   # divide by VOXEL_SIZE (1 mm)

MELANIN_CONDITIONS = {
    #                   µa (true × scale)        µs' (true × scale)   g     n
    'fair':  opt(0.020 * _EPI_SCALE, 1.80 * _EPI_SCALE, 0.80, 1.40),  # Fitzpatrick I-II,   f_mel ~1.3%
    'olive': opt(0.070 * _EPI_SCALE, 1.90 * _EPI_SCALE, 0.80, 1.40),  # Fitzpatrick III-IV, f_mel ~4.4%
    'dark':  opt(0.200 * _EPI_SCALE, 2.00 * _EPI_SCALE, 0.80, 1.40),  # Fitzpatrick V-VI,   f_mel ~16%
}
EPIDERMIS_LABEL = 15

# Map tissue name → (stl_path, label_id, optical_props [mua, mus, g, n])
# Optical properties at 650 nm, units mm⁻¹, mus is transport scattering
TISSUES = {
    "synovial":     (None,                                            14, opt(0.002,  0.02,  0.90, 1.36)),  # scaled from 808nm corrected
    "skin":         (None,                                            13, opt(0.011,  1.50,  0.80, 1.40)),
    "adipose":      (None,                                            12, opt(0.003,  1.20,  0.90, 1.44)),
    "muscle":       (None,                                            11, opt(0.0280, 0.60,  0.93, 1.37)),
    "pat1-cart":    (mesh_dir / "patella_lig_raw.stl",                10, opt(0.025,  1.80,  0.90, 1.37)),  # upper range fibrocartilage/ligament
    "pat2-cart":    (mesh_dir / "patella_cartilage_raw.stl",          10, opt(0.025,  1.20,  0.90, 1.37)),  # upper range hyaline
    "mtc-cart":     (mesh_dir / "tibia_cartilage_med_raw.stl",         9, opt(0.025,  1.20,  0.90, 1.37)),
    "ltc-cart":     (mesh_dir / "tibia_cartilage_lat_raw.stl",         8, opt(0.025,  1.20,  0.90, 1.37)),
    "fc-cart":      (mesh_dir / "femur_cartilage_raw.stl",             7, opt(0.025,  1.20,  0.90, 1.37)),
    "mm-men":       (mesh_dir / "men_med_raw.stl",                     6, opt(0.014,  2.00,  0.90, 1.37)),
    "lm-men":       (mesh_dir / "men_lat_raw.stl",                     5, opt(0.014,  2.00,  0.90, 1.37)),
    "patella-bone": (mesh_dir / "patella_raw.stl",                     4, opt(0.068,  2.80,  0.92, 1.37)),
    "fibula-bone":  (mesh_dir / "fibula_raw.stl",                      3, opt(0.068,  2.80,  0.92, 1.37)),
    "tibia-bone":   (mesh_dir / "tibia_raw.stl",                       2, opt(0.068,  2.80,  0.92, 1.37)),
    "femur-bone":   (mesh_dir / "femur_raw.stl",                       1, opt(0.068,  2.80,  0.92, 1.37)),
}

# Source positions in centered world coordinates (mm), relative to mesh center (0,0,0)
# Source positions are snapped to closest epidermis surface voxel.
SRC_WORLD_CONFIGS = [
    {'name': 'Source 1', 'world_pos': [2.7, -43, -10], 'color': 'red'  },
    {'name': 'Source 2', 'world_pos': [ -30,  26, -16], 'color': 'green'},
    {'name': 'Source 3', 'world_pos': [  32,  19, -17], 'color': 'blue' },
...
# Physical grid dimensions in mm
...
AUTO_ORIENT    = True          # auto-detect and correct Z-axis inversion
...
MUSCLE_THICK_MM  = 12   # concentric muscle shell around bone/cartilage assembly
ADIPOSE_THICK_MM =  6   # subcutaneous fat layer outside muscle
SKIN_THICK_MM    =  3   # dermis layer (epidermis relabelled separately)
...
def build_label_volume(tissues, res, spacing):
    ...
    if AUTO_ORIENT and femur_z_mean is not None and tibia_z_mean is not None:
        if femur_z_mean < tibia_z_mean:
            z_flip = True
            print(f"  [ORIENT] Z-axis inversion detected: femur_z={femur_z_mean:.1f} mm < "
    ...
    print(f"  Mesh bounding box: {mesh_dims.round(1)} mm")
    print(f"  Mesh center: {mesh_center.round(1)} mm")
...
def add_epidermis_layer(vol, skin_label, epidermis_label):
    ...
    resolution this maps to 1 voxel.  Optical properties set per
...
def find_joint_line_z(vol, tissues, origin, spacing, mesh_center):
    ...
    same sign convention as world_pos[2] in SRC_WORLD_CONFIGS (positive = superior).

    Method: find the Z slice with the most cartilage + synovial voxels.
    Placing sources at this height minimises oblique path length from source
    to joint, maximising fluence at the target tissues.
...
def find_valid_source_positions(vol, origin, spacing, mesh_center):
    ...
    if len(epi_coords) > 0:
        epi_world = origin + (epi_coords + 0.5) * spacing - mesh_center
        print(f"\nEpidermis surface (label {EPIDERMIS_LABEL}) extent:")
        print(f"  X: {epi_world[:,0].min():.1f} to {epi_world[:,0].max():.1f}")
        print(f"  Y: {epi_world[:,1].min():.1f} to {epi_world[:,1].max():.1f}")
        print(f"  Z: {epi_world[:,2].min():.1f} to {epi_world[:,2].max():.1f}")
...
def run_pmcx(vol, tissues, pmcx_source,
             source_power_mw=120,
             wavelength_m=650e-9,
             modulation_hz=40,
             duty_cycle=0.25,
    ...
    print(f"  Fluence range (mW/cm²): {nonzero.min():.3e} – {nonzero.max():.3e}")
...
TISSUE_COLORS = {
    ...
    11: "rgba(180,60,60,0.40)",     # muscle
    12: "rgba(255,220,150,0.40)",   # adipose
    13: "rgba(210,180,140,0.30)",   # skin
    14: "rgba(173,216,230,0.50)",   # synovial
    15: "rgba(255,200,180,0.50)",   # epidermis
}
...
def voxel_to_centered_world(vox_pos, origin, spacing, mesh_center):
    """Convert voxel index to centered world coordinates."""
...
def add_source_traces(fig, pmcx_source_plus, origin, mesh_center, arrow_length=20):
...
def plot_results(vol, fluence_combined, fluence_list, all_fluences, fluence_names,
                 tissues, origin, spacing, pmcx_source_plus, mesh_center,
                 smooth_sigma=1.0, plot_stride=3, title=""):
    ...
    def prep_fluence(flu, vol_p, name=""):
        ...
        if len(nonzero_flu) == 0:
            ...
            return np.full_like(flu_ds, -300.0)
        ...
        if smooth_sigma > 0:
            ...
            flu_log = np.where(weight_mask > 0.1, flu_log_smoothed, sentinel_val)
    ...
    fig = add_source_traces(fig, pmcx_source_plus, origin, mesh_center, arrow_length=20)
    fig._n_tissue_traces = n_tissue_traces_added

    fig.update_layout(
        title=title or "650 nm — Multi-tissue voxel volume + pmcx fluence",
        scene=dict(
    ...
    return fig, n_tissue_traces_added
...
def analyze_fluence_absorption(fluence, vol, tissues, voxel_size_mm, pmcx_source):
    ...
    groups = {
        ...
        'Epidermis': [n for n in results if 'epidermis' in n],
    }
...
def write_interactive_html(fig, tissues, all_fluences, fluence_names,
    ...
    # Trim to actual rendered traces (plot_results skips tissues with no voxels)
    tissue_info = tissue_info[:n_tissues]
...
<style>
    #controls {
        ...
        padding: 10px; max-width: 220px; color: #e6edf3;
        font-family: Arial, sans-serif;
    }
    #controls h4 { margin: 0 0 8px 0; font-size: 12px; color: #8b949e;
                   text-transform: uppercase; letter-spacing: 0.5px; }
    .tissue-row { display: flex; align-items: center; margin: 4px 0; gap: 8px; }
    .toggle-btn { width: 36px; height: 20px; border-radius: 10px; border: none;
                  cursor: pointer; position: relative; transition: background 0.2s;
                  flex-shrink: 0; }
    .toggle-btn.on  { background: #238636; }
    .toggle-btn.off { background: #484f58; }
    .toggle-btn::after { content: ''; position: absolute; width: 14px; height: 14px;
                         border-radius: 50%; background: white; top: 3px;
                         transition: left 0.2s; }
    .toggle-btn.on::after  { left: 18px; }
    .toggle-btn.off::after { left: 3px; }
    .tissue-label { font-size: 11px; color: #e6edf3; }
    .btn-group { display: flex; gap: 4px; margin-top: 6px; }
    .btn-group button { flex: 1; background: #21262d; color: #e6edf3;
                        border: 1px solid #30363d; border-radius: 4px;
                        padding: 3px 6px; font-size: 10px; cursor: pointer; }
    .btn-group button:hover { background: #30363d; }
    #fluence-select { margin-top: 10px; border-top: 1px solid #30363d; padding-top: 8px; }
    #fluence-select h4 { margin: 0 0 6px 0; font-size: 12px; color: #8b949e;
                         text-transform: uppercase; }
    #fluence-select select { width: 100%; background: #21262d; color: #e6edf3;
                             border: 1px solid #30363d; border-radius: 4px;
...
<div id="controls">
    ...
    <div id="fluence-select">
        ...
        <select id="fluence-dropdown" onchange="switchFluence(this.value)"></select>
    </div>
</div>"""
...
<script>
    ...
    for (let j = 0; j < nFluence; j++) { visState[nTissues + j] = (j === 0); }
    ...
    function switchFluence(idx) {
        ...
        for (let j = 0; j < nFluence; j++) { visState[nTissues + j] = (j === active); }
        applyVisibility();
    }
...
if __name__ == "__main__":
    ...
    # ── Step 2: Add synovial fluid and wrapping layers BEFORE source placement ─
    # This ensures sources snap to the outer epidermis surface, not to bare bone.
    print("\n=== Step 2: Adding tissue layers ===")
    ...
    # ── Step 4: Loop over melanin conditions ──────────────────────────────────
    for melanin_condition in ('fair', 'olive', 'dark'):
        print(f"\n{'=' * 60}")
        print(f"  Melanin condition: {melanin_condition}")
        print(f"{'=' * 60}")

        # Build a per-condition tissues dict with the correct epidermis props
        tissues_mc = dict(TISSUES)
        tissues_mc["epidermis"] = (None, EPIDERMIS_LABEL,
                                   MELANIN_CONDITIONS[melanin_condition])

        # ── Step 4a: Run or load fluence ──────────────────────────────────────
        print(f"\n=== Fluence ({melanin_condition}) ===")
        if FLUENCE_OUTPUT is not None:
            fluence_combined = np.load(f"fluence_combined_650nm_{melanin_condition}.npy")
            fluence_list     = [np.load(f"fluence_src{i+1}_650nm_{melanin_condition}.npy")
                                for i in range(len(PMCX_SOURCE))]
        else:
            fluence_combined, fluence_list = run_pmcx(
                vol, tissues_mc, PMCX_SOURCE
            )
            np.save(f"fluence_combined_650nm_{melanin_condition}.npy", fluence_combined)
            for i, fl in enumerate(fluence_list):
                np.save(f"fluence_src{i+1}_650nm_{melanin_condition}.npy", fl)

        all_fluences  = ([fluence_combined] + fluence_list
                         + [np.zeros_like(fluence_combined)])
        fluence_names = (["All Sources"]
                         + [src['name'] for src in PMCX_SOURCE_PLUS]
                         + ["No Sources"])

        # ── Step 4b: Absorption analysis ──────────────────────────────────────
        analyze_fluence_absorption(
            fluence_combined, vol, tissues_mc, VOXEL_SIZE, PMCX_SOURCE
        )

        # ── Step 4c: Plot and write HTML ───────────────────────────────────────
        print(f"\n=== Plotting ({melanin_condition}) ===")
        fig, _ = plot_results(
            vol, fluence_combined, fluence_list, all_fluences, fluence_names,
            tissues_mc, origin, VOXEL_SIZE, PMCX_SOURCE_PLUS, mesh_center,
            title=f"650 nm — {melanin_condition.capitalize()} skin"
        )

        output_path = f"fluence_overlay_650nm_{melanin_condition}.html"
        write_interactive_html(fig, tissues_mc, all_fluences, fluence_names,
                               PMCX_SOURCE_PLUS, output_path=output_path)

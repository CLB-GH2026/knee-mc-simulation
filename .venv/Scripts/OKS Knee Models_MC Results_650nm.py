...
base_dir = Path()
mesh_dir = base_dir / 'Raw_Mesh_Files_OKS001'
...
def run_subject(subject_id, mesh_dir_base, output_dir, melanin_condition='fair'):
    ...
    # Update TISSUES paths for this subject
    tissues = {
...
SOURCE_POWER_MW   = 160    # peak power per source (mW)
SOURCE_DUTY_CYCLE = 0.25   # modulation duty cycle
...
def find_joint_line_z(vol, tissues, origin, spacing, mesh_center):
    ...
    path length from source to joint regardless of subject anatomy.
...
def run_pmcx(vol, tissues, src_cfg, pmcx_source_list,
             source_power_mw=120, wavelength_m=650e-9,
             modulation_hz=40, duty_cycle=0.50, opt_eff=0.85):
...
def plot_depth_histogram(bin_centers, mean_flu, subject_id, bin_width_cm=0.25,
    ...
    fig.update_layout(
        title=dict(
            text=f'Fluence Rate vs Penetration Depth — {subject_id} (650 nm)',
            font=dict(size=14),
        ),
        xaxis=dict(
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR  = Path(f"results_650nm_{RUN_ID}")
    ...
    for condition in ['fair']:  # TEMP: full run uses MELANIN_CONDITIONS
        ...
        if cond_results:
            csv_path = OUTPUT_DIR / f"MC_Analysis_650nm_{condition}.csv"
    ...
    melanin_comparison_to_csv(
        ...
        output_path=str(OUTPUT_DIR / "MC_Melanin_Comparison_650nm.csv"),
        wavelength_nm=650,
    )

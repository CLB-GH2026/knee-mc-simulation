...
# Optical properties at 808 nm, units mm⁻¹, mus is transport scattering
TISSUES = {
...
def plot_results(vol, fluence_combined, fluence_list, all_fluences, fluence_names,
    ...
    fig.update_layout(
        title=title or "808 nm — Multi-tissue voxel volume + pmcx fluence",
        scene=dict(
...
if __name__ == "__main__":
    ...
    np.save("label_volume_808nm.npy", vol)
    ...
    for melanin_condition in ('fair', 'olive', 'dark'):
        ...
        if FLUENCE_OUTPUT is not None:
            fluence_combined = np.load(f"fluence_combined_808nm_{melanin_condition}.npy")
            fluence_list     = [np.load(f"fluence_src{i+1}_808nm_{melanin_condition}.npy")
                                for i in range(len(PMCX_SOURCE))]
        else:
            fluence_combined, fluence_list = run_pmcx(
                vol, tissues_mc, PMCX_SOURCE
            )
            np.save(f"fluence_combined_808nm_{melanin_condition}.npy", fluence_combined)
            for i, fl in enumerate(fluence_list):
                np.save(f"fluence_src{i+1}_808nm_{melanin_condition}.npy", fl)
        ...
        fig, _ = plot_results(
            ...
            title=f"808 nm — {melanin_condition.capitalize()} skin"
        )

        output_path = f"fluence_overlay_808nm_{melanin_condition}.html"
